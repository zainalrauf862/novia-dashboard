...
echo
echo "=== 2) Pasang auto-update: cron akan ambil kode + data tiap 10 menit ==="
cat > /etc/cron.d/novia <<'CRON'
SHELL=/bin/bash
TZ=Asia/Jakarta
*/10 * * * * root cd /opt/novia && git fetch -q origin && git reset -q --hard origin/main && /usr/bin/python3 pull_novia.py >/opt/novia/pull.log 2>&1 && cp -f /opt/novia/novia-dashboard.html /opt/novia/index.html
CRON
...
echo
echo "=== SELESAI. Mulai sekarang update kode OTOMATIS tiap 10 menit. ==="
